...
def login_and_go_to_badge(user_name,assignment_id)
...
def change_good_reviewer_badge_threshold(threshold)
  expect(page).to have_content "Good Reviewer"
  fill_in 'assignment_form_assignment_badge_1_threshold', with: '99'
  expect(page).to have_content("Save")
  click_button "Save"
end

def change_good_teammate_badge_threshold(threshold)
  expect(page).to have_content "Good Teammate"
  fill_in 'assignment_form_assignment_badge_2_threshold', with: '99'
  fill_in badge_2_threshold, with:threshold
  click_button "Save"
end

describe "Good Reviewer Badge" do
  before(:each) do
    assignment_setup
    # create(:assignment, name: 'Badge Assignment', directory_path: 'badge_test')
  ...
  it "should assign a badge to student" do
    login_and_go_to_badge("instructor6",1)
    visit "/participants/list?id=1&model=Assignment"
    expect(page).to have_content "Assignment Participants"
    visit "/assignments/1/edit"
    click_on "Badges"
    click_on "Save"
    expect(page).to have_content "Good Reviewer"
    find('tr', text: 'student2064').should have_content("Good Reviewer")
    # expect()
  end
end
#
...
# end
#   it "should assign a badge to student" do
#     login_and_go_to_badge("instructor6",@assignment)
#     visit "/assignments/1/edit"
#     expect(page).to have_content "Badges"
#     click_on "Badges"
#     change_good_teammate_badge_threshold(96)
#     # change_good_teammate_badge_threshold(90)
#     visit "/participants/list?id=#{assignment.id}&model=Assignment"
#     # page.all('table#plist td.exp').each |td|
#     #     if expect(td).to have_contain?("student5000")
#     #       expect(td).to have_css("img[src*='Good-Teammate-Badge.png']")
#     #     end
#   end
#
# end